#!/bin/bash
#SBATCH --job-name=DoradoCUDA_80_sepBarC_Fail
#SBATCH --ntasks=1
#SBATCH --mem=100G
...
$POD5_DIR \
--verbose --recursive --no-trim -x cuda:all | samtools fastq -@  $SLURM_CPUS_PER_TASK -T '*' - | pigz -p $SLURM_CPUS_PER_TASK > ${PREFIX}_fail.fastq.gz
